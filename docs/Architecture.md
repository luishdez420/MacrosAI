...
- `/saved-foods`: favorite/recent food management, including private organization tags on favorites.
...
- `/meal/[id]`: meal detail with date/time, source-backed food, portion, preparation-label, and added-oil editing; duplicate, split, reorder, removal/restore, source review, and deletion.
...
- `/recipes`: saved-recipe library with source-backed snapshot totals, category, local ingredient-aware search, meal-time/folder filtering, recent/use/name sorting, an account-private favorite flag, account-private tags, and one optional account-private folder per recipe. Users can toggle favorites, create/rename/delete folders, then assign/unfile a loaded recipe; folder deletion requires confirmation and only removes those folder links. The library also supports edit-through-Meal-Builder, log-to-today, and deletion. These display controls do not mutate recipe snapshots. Logging creates a new meal with the recipe's saved category.
...
- `GET /api/v1/export`: current-user JSON export for profile, preferences, goals, weight entries, hydration entries, meals, recipes, favorites with private tags, recents, and custom foods.
...
- `GET /api/v1/health/ready` checks database/schema readiness, the configured shared Redis limiter/provider circuit, and required aggregate worker heartbeats after startup. It returns only dependency category/health and a request ID on `503`; it does not expose connection values, Redis errors, process identifiers, or hostnames.
...
- Meal items.
- Meal images.
- Recipes, recipe items, account-private recipe favorites/tags/folders.
- Favorite foods and account-private favorite tags.
- Recent foods.
...
Not every modeled entity has a complete API and mobile workflow yet. Data correction reports have mobile source submission, current-user safe history, and a server-only Clerk-admin triage/resolution API; a dedicated staff UI, notification delivery, and moderation policy remain pending. Meal images support private temporary camera-job inputs plus opt-in confirmed-meal retention, deletion metadata, owner-only expiring access, and user-initiated deletion; configured cloud storage validation and a mobile image viewer remain pending. Weight entries have basic API and Profile logging/history support. Nutrition goals retain an optional direction alongside each effective-dated revision, and `GET /insights/range` calculates a descriptive weight comparison from entries inside the requested window plus the revisions effective on those dates. It reports evidence, data sufficiency, and whether stored directions were consistent, changed, or unavailable; it never makes a health prediction. Older goal revisions without a stored direction remain unavailable rather than being inferred from the current preference. Hydration entries are a separate, optional per-user/per-day total exposed on Today through `GET`, `PUT`, and `DELETE /api/v1/hydration/{date}`; the implementation deliberately has no default daily target, history, or insight model. Favorite foods can be added/removed from food provenance, displayed in Manual Search, and organized with user-private tags in Saved Foods. Tags attach to `FavoriteFood` through private tag/assignment tables, are returned only to their owner, and are included in that owner's export; they never mutate a `FoodSourceRecord`. Recent foods are persisted automatically from logged meal items, displayed in Manual Search, and removable from Saved Foods. Recipes have client-side ingredient-aware search, category/folder filtering, recent/use/name ordering, a private favorite flag, account-private tags, and one optional account-private folder. Recipe favorites, tags, and folders are owner-scoped and exported with the recipe without changing recipe or meal-item snapshots. Folder deletion unfiles only that owner's recipes. Recommendation-based grouping, shared collections, and server-side recipe search/pagination remain pending.
...
- Successful provider search and detail lookups are stored as normalized source records. For non-user providers, the initial normalized record and each later meaningful source-data change create a `FoodSourceRevision`; unchanged retrievals refresh freshness without adding noise. Authenticated `GET /foods/{id}` returns the latest five revisions as source provenance only, while meal-item snapshots remain immutable. Global provider records are shared between signed-in accounts; `provider=user` custom records require the owning account and are excluded from the authenticated catalog search/cache path. Food detail requests check accessible stored records before external providers. Provider searches also write a short-lived query-to-source-record index, so a complete fresh result set, including a cached no-result response, can be returned without re-calling the provider. The index stores source-record IDs only, expires after `FOOD_SEARCH_CACHE_TTL_SECONDS` (900 seconds by default), and is bypassed if a referenced provider record is stale or missing. Exact fresh food-name search matches and cached provider-failure fallback remain available.
...
7. Mobile posts a meal to `POST /api/v1/meals`. The API accepts an optional caller-owned `Idempotency-Key`; a user-scoped ledger compares a canonical request fingerprint and replays only an exact completed request. Reusing a key with changed content returns `409`, and meals retain a user/key uniqueness constraint as a second persistence boundary. The same ledger protects paid camera/label analysis, custom-food creation, source-correction-report creation, recipe creation, recipe logging, and daily weight upserts. Correction-report replay stores only the created owned-resource link and rebuilds the reporter-safe response, avoiding duplicate ledger storage of free-form report text. Camera Confirmation, Manual Search, Barcode, Natural Entry, Custom Food, Meal Builder, recipe-library, and Profile weight actions derive screen-scoped keys from confirmed content for safe retries. The retention worker removes expired replay records in bounded oldest-first batches.
...
Meal editing replaces meal items through `PATCH /api/v1/meals/{mealId}` only with the current quoted `If-Match` revision. The API conditionally claims that revision before writing, returns `409` without changes for a stale editor, and the mobile editor makes the person explicitly reload the latest meal instead of force-overwriting it. Saved-item edits preserve a source-food per-100g basis in the nutrient snapshot. A user-confirmed preparation value is descriptive only; confirmed added oil is a separate snapshot field and is added at 9 kcal and 1 g fat per gram, so changing a food portion or splitting an item cannot accidentally rescale or duplicate that oil.
...
The API applies rolling-window limits to authenticated `GET /foods/search`,
...
uses an IP-only `RATE_LIMIT_FOOD_SEARCH_*` policy for the shared signed-in catalog;
...
response status, rate-limit policy/outcome, dependency category, static worker type, provider name, provider
operation/outcome, and cache operation/outcome. They include request counts, duration buckets,
rate-limit decisions, aggregate background-worker liveness, provider outcome/latency/circuit state, food-cache hit/refresh/fallback
...
- Independently runnable maintenance-worker commands: `npm run api:analysis-worker` for durable camera review processing and `npm run api:food-source-refresh-worker` for bounded stale provider-record refresh. Docker Compose starts both plus the image-retention worker. Each worker writes only a random process heartbeat; the API exposes aggregate type-level liveness through readiness and protected metrics, never the process ID or host. Production startup requires all three worker types to remain fresh within the configured timeout; preview keeps the check advisory because `dev:phone` starts only the analysis worker. The retention worker also cleans expired analysis jobs, audit events, short-lived idempotency replay records, abandoned AI-quota reservations, and stale heartbeat rows in bounded batches. Reservation cleanup only releases expired capacity; it cannot dispatch or refund provider work.
...
- A free personal-preview Render Blueprint at `render.yaml` for one sleeping API and temporary Postgres database, plus [[deployment/render-free-preview-setup|its setup guide]]. It disables paid AI before image storage/provider calls and intentionally omits workers, Redis, R2, Sentry, and audit delivery. The separate `render.production.yaml` retains the selected paid Render API/PostgreSQL/Key Value topology and its three workers; [[deployment/render-r2-setup|its Cloudflare R2 guide]] keeps secrets and trusted-proxy values out of source control. Neither topology has been applied to a Render account yet.
