...
`GET /health/ready` is the readiness probe for deployments. It verifies the database connection/schema and, when configured for Redis, checks both the shared rate limiter and provider circuit breaker. Production also requires current anonymous heartbeats from the independently deployed meal-analysis, image-retention, and food-source-refresh workers. It returns `200` with dependency categories only when ready, or a request-correlated `503` without connection details when a required dependency is unavailable. The `backgroundWorkers` result exposes only required/healthy state by worker type, never process IDs, hostnames, user data, or storage paths. Local preview leaves worker heartbeats advisory and reports that status as `disabled`; it is not proof of shared-production operation.

`GET /metrics` returns Prometheus text only when `METRICS_ENABLED=true`. In production the configuration requires `METRICS_BEARER_TOKEN`; callers must provide it as `Authorization: Bearer <token>`, otherwise the endpoint returns `404`. The response contains normalized HTTP request counts/duration buckets, rate-limit policy decisions, provider operation outcomes/latency/circuit state, food-cache events, and latest readiness dependency gauges, including aggregate background-worker liveness by static worker type. It intentionally omits request IDs, user IDs, IP addresses, tokens, raw paths, food queries, barcodes, image data, exception messages, hostnames, and process IDs. Each API replica owns its own in-memory series, so a deployment collector must scrape and aggregate each replica. Local preview does not enable metrics by default.
...
`GET /account/ai-usage` returns a signed-in account's current camera and nutrition-label capacity before an image is submitted. Each operation reports remaining/total operation, image, and concurrent capacity, an `available` boolean for one new action, and an optional `nextAvailabilityAt` when a rolling-window record or active reservation can release capacity. The response intentionally omits entitlement tier, billing status, idempotency keys, request history, image content, prompts, provider output, and another account's usage. It is advisory only: the reservation made by `POST /meal-analysis/jobs`, `POST /meal-analysis`, or `POST /foods/label-analysis` is the authoritative quota decision and can still deny a request if capacity changes.
...
- `PUT /foods/favorites/{foodId}/tags`
...
`GET /foods/favorites`, `PUT /foods/favorites/{foodId}`, and `DELETE /foods/favorites/{foodId}` power favorite-food persistence. Favorites are added or removed from the food provenance screen and shown in Manual Search. `PUT /foods/favorites/{foodId}/tags` replaces the authenticated user's private tag list for an existing favorite with up to ten non-empty labels of 48 characters or fewer. Favorite responses and `GET /export` include `savedTags`; tags never modify shared USDA/Open Food Facts/custom source records or historical meal snapshots. A missing favorite, including another user's favorite, returns the standard owner-safe `404`.
...
- `GET /recipes`
- `GET /recipes/folders`
- `POST /recipes/folders`
- `PATCH /recipes/folders/{id}`
- `DELETE /recipes/folders/{id}`
- `GET /recipes/{id}`
- `PATCH /recipes/{id}`
- `PUT /recipes/{id}/tags`
- `DELETE /recipes/{id}`
...
`GET /insights/range` returns one row per calendar day for an inclusive 1-to-366-day window. Each daily row includes the calorie target effective on that date, daily totals, meal count, and goal-day status calculated from persisted meal-item snapshots. The summary includes the target effective on the final requested date plus logged-day count, goal-day count, and average calories, protein, and fiber across logged days. `weightComparison` is a descriptive server-calculated view of only the saved weight entries inside the selected dates: it returns entry count, first/last check-in dates, observation days, gram change, trend, and `insufficient_data`, `limited`, or `observed` status. Two entries are required to calculate a change; three entries spanning at least seven days are required for `observed`. It also reports the count of effective goal revisions and whether stored goal directions were consistent, changed, or unavailable. It never predicts an outcome or makes medical guidance. It rejects an end date before the start date and windows longer than 366 days.
...
`PUT /goals` creates or updates an effective-date goal revision. When `startsOn` is omitted, the revision is effective today; a repeat save for the same date updates that revision. It accepts an optional `goalDirection` (`maintain`, `cut`, or `gain`) so the selected direction is preserved with the date-effective nutrition target for later descriptive range insights. `GET /goals` returns the revision effective today, while `GET /goals/history` returns the authenticated user's revisions newest first for the Profile goal schedule. The separate `goalDirection` preference remains the current Profile setting. Historical revisions created before this field have no stored direction and are reported as unavailable rather than inferred. A richer goal editor and production account lifecycle remain planned.
...
`GET /export` returns a `living-nutrition-export/v1` current-user JSON export containing profile/session metadata without a token, preferences, nutrition goals, weight entries, hydration entries, meals and recipes with item snapshots and private tags, favorite foods with their private `savedTags`, recent foods, and custom foods. It works for the authenticated internal profile in either Clerk or local-development mode. The mobile Data Controls flow shows a recipient warning and requires acknowledgement before handing this sensitive temporary-cache file to the native system share sheet; the app does not control any recipient copy after sharing.
...
Basic weight entry storage exists. `POST /weight` upserts one entry per user/date and is used for both creation and editing, `GET /weight` returns recent entries for the current user, and `DELETE /weight/{loggedOn}` deletes the current user's entry for that ISO date when it exists. `POST /weight` accepts an optional `Idempotency-Key`; an exact retry for the same user returns the original entry, while reuse with changed date, weight, or note returns `409`. The mobile Profile flow derives one key from the explicit date and weight payload so a retry after an ambiguous connection failure cannot create or overwrite a different daily entry.
